...
new _vue2.default({
	...
	computed: {
		...
		folder_context: function folder_context() {
			return [{ name: '排序方式', todo: this.Fuck }, { name: '刷新', todo: this.LoadDir }, { name: '全部还原', todo: this.ReCoverAll }, { name: '清空回收站', todo: this.DelAllFile }, { name: '属性', todo: this.Fuck }];
		}
	},
	methods: {
		// 未定义的右键方法
		...
		FileClick: function FileClick(index) {
			...
			function todo(index) {
				...
				if (list[index].sel) {
					...
					try {
						for (var _iterator = that.index[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
			...
			if (event.shiftKey) {
				...
				if (type == 'check' || event.ctrlKey) {
					todo(index);
					this.shift_bak = index;
				} else if (type == 'name' || type == 'icon') this.Fuck(index);
			}
		},
		...
		// 还原全部文件
		ReCoverAll: function ReCoverAll() {
			log('Fuck');
		},
		...
		DelFile: function DelFile() {
			...
			if (confirm('你真的忍心丢我们到黑黑的回收站吗(>_<)')) (0, _ajax_post2.default)('/Home/DeleteFile', { DeleteArray: JSON.stringify(json) }, function (data) {
